...
public class customerReservation {
    ...
    public void reserve(String ID) {
        ...
        String storeName = temp.get(storeIndex - 1);
        Map<String, Integer> noShowCustomers = new HashMap<>();

        while (database.store.hasNextLine()) {
            String[] str = database.store.nextLine().split("\t");
            if(str.length == 5 && str[0].equals(storeName)) {
                String[] noShowPart = str[4].split(" ");
                for(int j = 0; j < noShowPart.length; j+=2) {
                    noShowCustomers.put(noShowPart[j], Integer.parseInt(noShowPart[j+1]));
                }
                for (Map.Entry<String, Integer> entry : noShowCustomers.entrySet()) {
                    if (ID.equals(entry.getKey()) && entry.getValue() >= 3) {
                        System.out.println("'" + ID + "'님은 " + storeName + "의 블랙리스트 입니다. 다른 매장을 선택해주세요.\n"
                        );
                        return;
                    }
                }
...
//                }
            ...
            try {
                ...
                writer.println(newReservationInfo.substring(0, newReservationInfo.length() - 1));
